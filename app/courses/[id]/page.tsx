...
export default function CoursePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (staffData) { setStaff(staffData); if (staffData.length > 0) setCurrentStaffId(staffData[0].id) }
      setLoading(false)
    }
    fetchData()
  }, [courseId])
  ...
  const loadProgress = useCallback(async () => {
    ...
    const { data: progress } = await supabase.from('section_progress').select('section_id, check_passed').eq('staff_id', currentStaffId).eq('course_id', courseId)
    const passed: Record<string, boolean> = {}
    ;(progress || []).forEach((p) => { if (p.check_passed) passed[p.section_id] = true })
    setPassedSections(passed)
    const { data: exams } = await supabase.from('exam_attempts').select('passed').eq('staff_id', currentStaffId).eq('course_id', courseId)
    setExamPassedBefore((exams || []).some((e) => e.passed))
  }, [currentStaffId, courseId])
  useEffect(() => { loadProgress() }, [loadProgress])

  // Reset slide position when entering a slides section
  useEffect(() => {
  ...
  async function saveSectionProgress(sectionId: string, score: number, passed: boolean) {
    await supabase.from('section_progress').insert({ staff_id: currentStaffId, course_id: courseId, section_id: sectionId, check_score: score, check_passed: passed })
  }
  ...
  async function submitExam() {
    ...
    const passed = score >= (course?.pass_mark || 80)
    const { data: prev } = await supabase.from('exam_attempts').select('id').eq('staff_id', currentStaffId).eq('course_id', courseId)
    await supabase.from('exam_attempts').insert({ staff_id: currentStaffId, course_id: courseId, attempt_number: (prev?.length || 0) + 1, score, passed })
          ...
          <div style={pageStyle}>
            <div style={{ maxWidth: '520px', margin: '0 auto', textAlign: 'center', paddingTop: '20px' }}>
              <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.14)', borderRadius: '20px', padding: '32px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ fontSize: '13px', color: '#5A5A55', marginBottom: '4px' }}>This certifies you have completed</div>
          ...
          <div style={{ maxWidth: '900px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '24px' }}>
              {bySection.map((row) => {
                ...
                const label = row.pct >= 80 ? 'Strong' : row.pct >= 50 ? 'Review' : 'Needs work'
                return (
                  <div key={row.section.id} style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '10px', padding: '14px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      ...
                      <span style={{ fontSize: '13px', fontWeight: 700, color: colour }}>{row.pct}% · {label}</span>
                    </div>
          ...
          <div style={{ maxWidth: '100%' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
              <button
                ...
                style={{ ...secondaryBtn, marginTop: 0, opacity: slideIndex === 0 ? 0.4 : 1, cursor: slideIndex === 0 ? 'not-allowed' : 'pointer' }}
              >← Previous</button>
              ...
              {!isLastSlide ? (
                ...
                <button onClick={() => setSlidesSeen((s) => ({ ...s, [section.id]: true }))} style={{ ...startBtn, marginTop: 0, background: allSeen ? '#0F6E56' : '#2D5BE3' }}>
                  {allSeen ? '✓ All slides viewed' : '✓ Mark slides as viewed'}
                </button>
              )}
              ...
              <div style={{ display: 'flex', gap: '5px', marginLeft: 'auto' }}>
                {slideList.map((_, i) => (
                  <div key={i} style={{ width: '8px', height: '8px', borderRadius: '50%', background: i === slideIndex ? '#2D5BE3' : i < slideIndex || allSeen ? '#0F6E56' : '#D0CEC8' }} />
                ))}
              </div>
            </div>

            {/* Continue button — locked until all slides viewed */}
            {(() => {
              const locked = !allSeen
              const btnStyle = locked ? { ...startBtn, marginTop: 0, opacity: 0.4, cursor: 'not-allowed' } : { ...startBtn, marginTop: 0 }
              const label = qs.length > 0
                ? (locked ? 'View all slides to continue' : 'Continue to section check →')
                : (locked ? 'View all slides to continue' : (currentIndex < sections.length - 1 ? 'Next section →' : '✓ Finish sections'))
              return (
                <button onClick={async () => { if (locked) return; if (qs.length > 0) goToQuiz(); else { setPassedSections((p) => ({ ...p, [section.id]: true })); await saveSectionProgress(section.id, 100, true); moveNext() } }} disabled={locked} style={btnStyle}>{label}</button>
              )
            })()}
          </div>
        </div>
    ...
    return (
      <div style={pageStyle}>
        ...
        {(() => {
          ...
          const label = qs.length > 0 ? (locked ? `Watch the video first (${secondsLeft}s)` : 'Continue to section check →') : (locked ? `Watch the video first (${secondsLeft}s)` : (currentIndex < sections.length - 1 ? 'Next section →' : '✓ Finish sections'))
          return <button onClick={async () => { if (locked) return; if (qs.length > 0) goToQuiz(); else { setPassedSections((p) => ({ ...p, [section.id]: true })); await saveSectionProgress(section.id, 100, true); moveNext() } }} disabled={locked} style={btnStyle}>{label}</button>
        })()}
      </div>
    )
  }
