...
type Staff = {
  ...
  role: string; active: boolean; must_reset_password: boolean
}
type Department = { id: string; name: string; icon: string }

const ROLES = [
  { value: 'sw',      label: 'Support Worker', icon: '🧑‍⚕️', color: '#2D5BE3', bg: 'rgba(45,91,227,0.1)' },
  { value: 'manager', label: 'Manager',         icon: '👔',    color: '#534AB7', bg: 'rgba(83,74,183,0.1)' },
  { value: 'admin',   label: 'Admin',           icon: '⚙️',    color: '#854F0B', bg: 'rgba(133,79,11,0.1)' },
]
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ fontSize: '14px', color: '#8A8A82' }}>{staff.length} members · {activeCount} active · {staff.length - activeCount} inactive</p>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            ...
            <tbody>
              {staff.map(s => {
                ...
                return (
                  <tr key={s.id} style={{ borderBottom: '1px solid rgba(0,0,0,0.06)', opacity: s.active ? 1 : 0.55 }}>
                    <td style={td}><strong>{s.name}</strong></td>
                    <td style={td}><span style={{ color: '#5A5A55' }}>{s.email}</span></td>
                    <td style={td}>{s.job_title || '—'}</td>

                    {/* Inline role select styled as coloured badge */}
                    <td style={td}>
                      <select
                        value={s.role}
                        onChange={e => changeRole(s.id, e.target.value)}
                        style={{ padding: '3px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: 700, border: `1.5px solid ${rs.color}`, background: rs.bg, color: rs.color, cursor: 'pointer', outline: 'none' }}>
                        {ROLES.map(r => <option key={r.value} value={r.value}>{r.icon} {r.label}</option>)}
                      </select>
                    </td>
                        ...
                        {departments.map(dept => {
                          ...
                          return (
                            <button key={dept.id} type="button" onClick={() => toggleDept(s.id, dept.id)}
                              style={{ padding: '2px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: 600, cursor: 'pointer', border: on ? '1.5px solid #0F6E56' : '1.5px solid rgba(0,0,0,0.1)', background: on ? 'rgba(15,110,86,0.08)' : '#F8F7F4', color: on ? '#0F6E56' : '#8A8A82' }}>
                              {dept.icon} {dept.name}
                            </button>
                          )
                        })}
                    ...
                    <td style={td}>
                      <button type="button" onClick={() => toggleActive(s.id, s.active)}
                        style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 600, border: 'none', cursor: 'pointer', background: s.active ? 'rgba(15,110,86,0.1)' : 'rgba(153,60,29,0.08)', color: s.active ? '#0F6E56' : '#993C1D' }}>
                    ...
                    <td style={td}>
                      <button type="button" onClick={() => toggleForceReset(s.id, s.must_reset_password)}
                        style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 600, border: 'none', cursor: 'pointer', background: s.must_reset_password ? 'rgba(153,60,29,0.08)' : 'rgba(0,0,0,0.05)', color: s.must_reset_password ? '#993C1D' : '#8A8A82' }}>
